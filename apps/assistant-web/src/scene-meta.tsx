...
export const assistantScenes: Record<AssistantScene['key'], AssistantScene> = {
  chat: {
    ...
    title: 'AI 销售工作台',
    subtitle: '计划驱动工作台',
    headline: '先理解目标并生成建议计划，再由用户裁剪、确认和逐步推进。',
    description: '从这里描述销售目标、上传材料或选择场景入口。系统会调用真实智能体接口编排任务，不再生成本地示例结果。',
    defaultInput: '描述你的销售目标，或输入 / 选择场景命令',
    prompts: [
      { key: 'p1', label: '上传拜访录音或纪要，生成处理计划', description: '让智能体识别材料、生成步骤，并在需要写回时等待确认。' },
    ...
    hotTopics: [
      { key: 'h1', title: '如何开始一个真实销售任务？', description: '描述目标、上传材料或使用 slash 命令，系统会走真实智能体接口。' },
    ...
    guides: [
      ...
      { key: 'g3', title: '可观测', description: '真实追踪返回后会展示意图帧、计划和工具调用。' },
    ],
    taskCards: [],
  },
  'customer-analysis': {
    ...
    headline: '客户分析入口会调用真实智能体接口，不提供本地示例卡片。',
    ...
    guides: [
      ...
      { key: 'rg2', title: '来源透明', description: '外部研究只做补强，所有引用都应带来源与追踪。' },
    ],
    taskCards: [],
  },
  'conversation-understanding': {
    ...
    headline: '会话理解既能单独使用，也能被复合计划动态加入。',
    description: '上传录音、转写或纪要，让智能体抽取关键事实、客户承诺事项和风险信号，供后续需求待办与问题陈述复用。',
    ...
    hotTopics: [
      ...
      { key: 'vh2', title: '它和复合计划的关系是什么？', description: '它可以作为录音处理计划的分析节点，也可以被单独调用。' },
    ],
    guides: [
  ...
  tasks: {
    ...
    route: '/chat/tasks',
    title: '我的任务',
    subtitle: '查看真实任务、资产、追踪和待确认写回',
    headline: '任务中心保留入口，但本轮不再展示本地示例任务。',
    description: '这里将承接真实智能体运行、资产和待确认写回。当前没有真实数据时会展示空态，不会加载示例任务。',
    defaultInput: '/我的任务',
    prompts: [
      { key: 't1', label: '/我的任务', description: '查看真实任务与资产状态。' },
      { key: 't2', label: '查看当前会话追踪', description: '打开运行洞察查看真实智能体追踪。' },
      { key: 't3', label: '查看待确认写回', description: '后续接入真实写回预览后展示。' },
    ],
    hotTopics: [
      { key: 'th1', title: '当前有真实任务数据吗？', description: '有真实智能体运行后会展示；无数据时保持空态。' },
      { key: 'th2', title: '追踪在哪里看？', description: '点击右上角运行洞察，查看当前会话最新真实追踪。' },
    ],
    guides: [
      { key: 'tg1', title: '不展示示例数据', description: '任务、资产和追踪均只展示真实接口返回。' },
      { key: 'tg2', title: '后续接入', description: '后续可接入真实任务列表、智能体运行列表和写回确认。' },
    ],
    taskCards: [],
  },
};
