...
| 业务对象 | 系统设置项 | 说明 |
|------|------|------|
| 客户 | `customer.templateId` | 租户配置的客户模板 ID |
| 联系人 | `contact.templateId` | 租户配置的联系人模板 ID |
| 商机 | `opportunity.templateId` | 租户配置的商机模板 ID |
| 商机跟进记录 | `followup.templateId` | 租户配置的跟进记录模板 ID |
...
```mermaid
flowchart TD
    A["系统设置读取 templateId"] --> B["调用表单模板接口"]
    B --> C["按官方流程获取 / 解析 codeId"]
    C --> D["提取字段定义、必填规则、字典、关联关系"]
    D --> E["叠加管理员补充规则"]
    E --> F["生成技能契约与执行映射"]
...
#### Step 1：从系统设置读取 templateId

系统不应扫描所有轻云对象后再盲目开放，而应先读取租户配置中已启用的模板 ID。
...
轻云实例读写通常还需要 `codeId` 参与，因此系统应按官方文档流程获取表单 `codeId`。
...
系统基于 `customer.templateId` 调用官方模板接口，拿到客户表单的真实字段结构。

### Step C：获取 codeId

系统按官方文档流程获取该模板关联的表单 `codeId`，写入对象注册表。
...
```json
{
  "skill_name": "shadow.customer.create",
  "description": "在轻云记录系统中创建客户",
  "when_to_use": "当用户明确要录入客户、创建客户、补录客户时使用",
  "not_when_to_use": "当用户只是查询客户情况或分析公司时不要使用",
  ...
  "output_card_type": "customer-create-preview",
  "source_object": "customer",
  "source_template_id": "tmpl_xxx",
  "source_code_id": "code_xxx"
}
```
