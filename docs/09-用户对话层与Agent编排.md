...
- “贝斯美上次拜访客户主要提了什么需求”这类问题会被识别为 `visit_needs`，优先输出“拜访需求摘要 / 客户主要需求”，不再套用泛化客户旅程模板。
...
- 拜访需求摘要取材优先级为 `完整客户需求工作待办分析 > 问题陈述 > 拜访会话理解 > 拜访录音 > 跟进记录`，公司研究和客户价值定位只作为背景，不进入“客户主要需求”主清单。
- 命中 `客户需求工作待办分析` 后，会通过 `artifact.get` 回读完整 Markdown，并优先解析 `一、客户核心需求` 下的 `需求 N` 结构，保留标题、背景、目标、关注点等信息，避免向量只命中单个 chunk 时只回答局部需求。
- 需求摘要会扩大 `analysis_material` 候选并按下游分析技能做多样性选择，保留 `客户需求工作待办分析`、`问题陈述`、`拜访会话理解` 等互补证据，避免同一技能的多个 chunk 挤掉关键材料。
- 需求条目只抽取明确客户需求、痛点和问题信号，过滤“项目/场景背景”“待补充材料/证据”等分析模板噪声。
- 若完整 Artifact 读取失败，摘要会回退到已有 snippet 抽取逻辑，并在 trace 中保留 `artifact.get` 失败状态。
- 若尚未检索到正式分析资料但已有录音资料包，摘要会优先从录音资料包中含业务信号的片段提炼需求和重点，同时提示需要补跑“拜访会话理解 / 客户需求工作待办分析”形成正式分析资料。
