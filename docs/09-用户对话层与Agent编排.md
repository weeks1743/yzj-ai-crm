...
- 用户 AI 端不再展示个人设置入口和 SOUL 设置页，`/settings/personal` 重定向回 `/chat`。
- 后端不再注册个人设置读写接口；历史 `agent_personal_settings` 表仅作为 legacy 数据保留，不在启动或迁移中自动删除。
- 左下角仅展示登录人头像和姓名，不再依赖个人设置接口回填显示名。
...
- 左侧会话使用官方 `Conversations` 菜单支持用户会话重命名；侧栏仅展示头像和姓名，不展示角色/SOUL 调试文案、个人设置入口或右上角固定“公司研究入口”标签。
