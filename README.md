...
`yzj-ai-crm` 从 `0.1.0` 起切换为同仓库协同开发的双系统正式原型，并在 `0.2.x` 引入独立 `admin-api` 承载系统基础设置、云之家组织同步和客户影子技能生成的真实联调；`0.3.0` 开始补入首个真实外部技能：
...
- 版本：`0.3.0`
- 目标：进入外部技能真实接入阶段，首轮落地 `ext.image_generate`
