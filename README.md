...
`yzj-ai-crm` 从 `0.1.0` 起切换为同仓库协同开发的双系统正式原型，并在 `0.2.x` 引入独立 `admin-api` 承载系统基础设置、云之家组织同步和客户影子技能生成的真实联调；`0.3.0` 开始补入首个真实外部技能；`0.4.0` 新增独立 `skill-runtime`；`0.4.1` 把外部技能市场接上 `skill-runtime` 的目录映射与统一调试台；`0.4.2` 进一步将 `ext.super_ppt` 以 Skill 形态接入 Docmee V2 API，并补齐编辑器链路：

- `管理员后台`：基于 `ant-design-pro` 工程范式，承担租户接入、记录系统治理、AI 资产管理、技能编排与系统配置
- `用户 AI 端`：基于 `ant-design/x` 独立式工作台，承担自然语言对话、任务推进、录音分析、公司研究与拜访准备
- `admin-api`：基于 TypeScript + Node，承担本地 `.env` 配置读取、SQLite 持久化、组织同步、客户模板元数据刷新、影子技能 bundle 生成、图片外部技能目录，以及外部技能市场对 `skill-runtime` 的统一聚合代理
- `skill-runtime`：基于 TypeScript + Node 的独立 SKILL Runtime 服务，负责扫描 `3rdSkill/*/SKILL.md`、建立 catalog、调用 DeepSeek 工具循环、接入火山联网搜索，并异步执行 `company-research`、PM 文本类 skill、`pptx` 与 `super-ppt` 等外部 skill
...
- 版本：`0.4.2`
- 目标：以 Skill 形态接入 `ext.super_ppt`，让 markdown 报告能通过 Docmee V2 API 生成并继续编辑 PPT
...
- [docs](./docs)：
  正式设计文档
- [iterations/0.4.2](./iterations/0.4.2/README.md)：
  当前版本说明
