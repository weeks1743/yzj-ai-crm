...
- `read_source_file` 兼容读取 `inputs/profile-analysis` 等输入子目录；目录不存在或没有可读文本时返回空输入，不再中断下游 Skill。
...
- 生产部署完成：
  - GitHub `main` 已部署到生产 HEAD `ff33c9b`。
...
- 生产录音任务 `recording-task-7b84c7af` 验收：
  ...
  - 部署补充修复后再次触发 `ext.customer_value_positioning_pm`，job `69127f4b-2840-4ca4-8ced-63ae32f1e4fd` 已 `succeeded`；`inputs/profile-analysis` 读取为空目录结果，不再失败；最终摘要未出现客户、商机、跟进记录内部 ID。
